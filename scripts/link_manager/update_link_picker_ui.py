...
# 1. リンクパネルのリストアイテム部分のデザインをフォルダ管理に合わせて更新
pattern = re.compile(r"\{\/\* リンク集パネル \*\/\}.*?\{\/\* フォルダ管理 \*\/\}", re.DOTALL)

updated_panel = """{/* リンク集パネル */}
          {showLinkPicker && (
            <div
              ref={linkPickerRef}
              ...
              <div style={{ background: '#f8fafc', padding: '8px 12px', borderRadius: '0 0 10px 10px', flexShrink: 0 }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:'6px', maxHeight:'250px', overflowY:'auto', paddingBottom:'4px' }}>
                  {customLinks.map((link, idx) => (
                    <div key={link.id} style={{ display: 'flex', alignItems: 'center', gap: '6px', background:'white', padding:'4px 6px', borderRadius:'6px', border:'1px solid #e2e8f0', boxShadow:'0 1px 2px rgba(0,0,0,0.02)' }}>
                      <a
                        href={link.url}
...
content = pattern.sub(updated_panel, content)
