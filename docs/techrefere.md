...
- README には現在仕様だけを書く。
- Adobe Express を使った時代の理由、やめた理由、captcha 問題、artifact での切り分け、成功 run の実測はこの `C:\Users\mahha\OneDrive\開発\Blog_Vercel\docs\techrefere.md` に集約する。
...
## 予約投稿システム仕様（2026-05-16 監視セッション改修版）
...
そのため、5 分おきや 2 分おきに監視しても、その監視 workflow 自体が遅れれば予約投稿も遅れる。さらに、予約ごとに cron を複数本作っても「起動口」を増やしているだけで、GitHub Actions 側が起動しなければ同じ問題が残る。今回の改修では「何度も cron を立ち上げる」考え方をやめ、1 回起動した監視ジョブがセッション中に何度も予約一覧を確認する方式へ変更した。
...
ただし GitHub Actions の `schedule` を定期監視として信用しない。予約登録時に、予約一覧から監視セッション用 workflow を生成し、予約群の少し前に dispatcher を 1 回だけ起動する。起動後は dispatcher 自身が一定間隔で予約一覧を見に行く。
...
生成される cron は、各予約に対して複数本ではなく、監視セッションの開始時刻に対して 1 本だけにする。

- 予約時刻の 35 分前に監視セッションを開始する
- 監視セッションは最大 330 分継続する
- セッション中は 60 秒おきに予約一覧を確認する
- 1 セッション内に複数の予約が収まる場合は、後続予約用の cron を増やさず同じ監視セッションで拾う

この方式では、cron は「投稿時刻そのものに起動するため」ではなく、監視セッションを前もって開始する入口である。起動後の確認は `C:\Users\mahha\OneDrive\開発\Blog_Vercel\.github\scripts\note_post_schedule_dispatch.py` の `NOTE_POST_MONITOR_MODE=true` が担当する。
...
- `NOTE_POST_MONITOR_DURATION_MINUTES`: `330`
- `NOTE_POST_MONITOR_POLL_SECONDS`: `60`
...
ただし旧方式と違い、常時 5 分監視や予約ごとの複数 cron に依存せず、1 回起動した監視セッション内で 60 秒おきに確認する。これにより、GitHub Actions の起動遅延が数分から数十分程度であれば、投稿時刻のズレを抑えやすくなる。
...
## note 公開設定画面の待機仕様（2026-05-16 追記）

### 背景

2026 年 5 月 16 日の手動投稿 run では、本文保存、OGP 展開、トップ画像アップロードまでは成功したが、`公開に進む` の後で `ハッシュタグ入力 を特定できませんでした` により失敗した。

対象記事は約 9,789 文字で、OGP 展開が 27 件、トップ画像アップロードも伴っていた。このため、note エディタから公開設定画面へ遷移した直後に、ハッシュタグ入力欄や `投稿する` ボタンがまだ描画されていない時間が発生しやすかった。

### 原因

旧実装では `C:\Users\mahha\OneDrive\開発\Blog_Vercel\scripts\pipeline\prompts\05-draft-manager\note_draft_poster.py` が `公開に進む` クリック後に 3.5 秒だけ固定待機し、その後 3 秒の候補探索でハッシュタグ入力欄を探していた。

今回の記事だけ急に失敗した主因は、ハッシュタグ文字列そのものではなく、重い記事で公開設定画面の描画が探索開始に間に合わなかったことだと判断する。

### 対策

`公開に進む` の後は、最大 45 秒まで公開設定画面を待つ。ただし 45 秒固定で待つのではなく、以下のいずれかが見えた時点で即座に次へ進む。

- `投稿する` ボタン
- `ハッシュタグ` またはタグ入力に関係する表示
- `マガジン` 表示
- `role=textbox`、`role=combobox`、`contenteditable` を含むタグ入力候補

失敗時には `C:\Users\mahha\OneDrive\開発\Blog_Vercel\.github\workflows\note-post.yml` から `NOTE_TOP_IMAGE_DEBUG=1` を渡し、`C:\Users\mahha\OneDrive\開発\Blog_Vercel\scripts\pipeline\debug\note_gazo_test\artifacts` の内容と `/tmp/note_post_result.json` を GitHub Actions artifact として保存する。
## 2026-05-25 Vercel Hobby 12 Function 上限への対応

### 背景
Vercel の Production Deployment が `No more than 12 Serverless Functions can be added to a Deployment on the Hobby plan` で失敗した。原因は `C:\Users\mahha\OneDrive\開発\Blog_Vercel\api` 配下の JavaScript が 14 個になっており、Vercel Hobby plan の 12 Function 上限を超えていたためである。

### 対応方針
機能は削除せず、Vercel が Serverless Function として数える公開入口だけを 12 個へ戻した。`C:\Users\mahha\OneDrive\開発\Blog_Vercel\api` 直下は HTTP 入口だけを置き、共通処理や統合APIの実装本体は `C:\Users\mahha\OneDrive\開発\Blog_Vercel\lib` へ移す。

### 実施内容
- `C:\Users\mahha\OneDrive\開発\Blog_Vercel\api\_onedrive-token-sync.js` を `C:\Users\mahha\OneDrive\開発\Blog_Vercel\lib\onedrive-token-sync.js` へ移動した。これは公開APIではなく、複数APIから import される共通処理である。
- `C:\Users\mahha\OneDrive\開発\Blog_Vercel\api\info-viewer-notion.js` を `C:\Users\mahha\OneDrive\開発\Blog_Vercel\lib\info-viewer-notion.js` へ移動した。Notion 保存機能自体は残し、公開入口を `/api/info-viewer?resource=notion` に統合した。
- 旧 `/api/info-viewer-notion` は `C:\Users\mahha\OneDrive\開発\Blog_Vercel\vercel.json` の rewrite で `/api/info-viewer?resource=notion` へ流し、外部互換を維持した。
- Xpost 系は以前からの方針通り、`C:\Users\mahha\OneDrive\開発\Blog_Vercel\api\xpost-blog.js` の `resource` 分岐に統合する。

### 今後のルール
新しい Vercel API が必要になった場合でも、まず既存 Function の `resource` 分岐へ統合できるかを検討する。`C:\Users\mahha\OneDrive\開発\Blog_Vercel\api` 直下へ `.js` を追加すると Function 数が増えるため、Hobby plan のままでは Production Deployment が失敗する可能性が高い。
