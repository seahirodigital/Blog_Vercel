...
### 2026-08-14: 同一商品ブロックの複数掲載を許可

確認した原因:

- `affiliate_links.txt` の `===adapter===` には19商品ブロックがあり、Anker Prime ドッキングステーションとAnker USB-C ＆ USB-Cケーブルがそれぞれ完全一致で2回記載されている。
- 従来の完成記事検査は、同一の全文ブロックを二度見つけると `商品ブロックが重複しています` で失敗させていた。
- 2026-08-13 23:45開始のバッチ `3653f219-f8e2-4849-9a36-299794e992fd` は、Insta360 X6とM5 iPad Proの両方が同じ `adapter` セクションを使ったため、同じAnker Prime商品で2件とも失敗した。MLXの二重生成や親記事の違いが原因ではない。

確定した修正:

- 同じ完成記事内に同一商品ブロックが複数回あっても許可する。
- 検査はカテゴリ共通説明文の末尾から開始し、一商品を検出するごとに検索開始位置をそのブロック末尾へ進める。
- `affiliate_links.txt` に同じ商品が2回あれば、完成記事にも入力順で2回必要とする。二回目の欠落は引き続き不合格にする。
- 入力で要求された回数より多く同じ商品が掲載されていても不合格にしない。親記事の別位置に同じ商品が元から存在する場合も許可する。

ローカル検証結果:

- Python単体テスト75件に合格した。
- 実際の `===adapter===` 19商品ブロックを入力順で組み立て、完全一致の商品が2回あっても完成記事検査に合格することを確認した。
- 入力商品群の後ろへ同一商品をさらに一回追加しても合格し、同一商品の掲載回数を上限で制限しないことを確認した。
- 入力が要求する同一商品の二回目だけを削除した記事は `調整済み商品ブロックが掲載されていません` で不合格となり、必要な回数と順序の検査が維持されることを確認した。
- Python構文、`git diff --check`、Vercel Productionビルドに合格した。

本番反映結果:

- 修正コミット `0d6b9713` を `main` へpushした。
- Vercel Productionデプロイ `dpl_CdiypjfaaBQQf5ETsDXDvS3ogdwW` が `Ready` となり、`https://blog-vercel-dun.vercel.app` へ反映された。
- 失敗済みバッチの状態は自動変更しない。修正後の検査を使うには、対象記事と充電器を再選択して新しいジョブを作成する。

### 2026-08-14: アフィリエイトタブの追加・改名・削除

確定した仕様:

- アフィリエイトリンク管理のタブ列右端に `+` を追加し、Blog Vercelから任意のカテゴリタブを作成する。
- `SSD` を作成した場合、表示タブ名と保存マーカーをともに `SSD` とし、`===SSD===` 直下へ入力本文を保存する。
- OneDriveのテキストを直接編集して追加・改名・削除したセクションはBlog Vercelの再読込でタブに反映し、Blog Vercelでの同じ操作はOneDriveの同一テキストへ反映する双方向同期とする。
- `MEMO1`、`MEMO2` は従来の表示名を維持し、カテゴリ名統一の対象外とする。
- カテゴリタブの右クリックに「名前を編集」「削除」を表示する。削除は本文も消えるため確認後に実行する。
- 既存の `battery`、`adapter`、`cable` は編集APIでそれぞれ `バッテリー`、`充電器`、`ケーブル` へ正規化し、次回保存後のテキストも日本語マーカーに統一する。
- `周辺機器DB` に旧英語セクション名が残っていても、Node.js側のプレビューとMLX・Gemini共通のPython抽出層が日本語マーカーを同一カテゴリとして解決する。

ローカル検証結果:

- 既存英語マーカーを編集用に読み込み、日本語タブ名と同じ日本語マーカーで再保存できることを自動テストで確認した。
- `===SSD===` の読込・保存、旧 `battery` 指定による `===バッテリー===` の抽出、既存三カテゴリの全商品順序保持に合格した。
- Blog VercelのJSX構文解析に合格した。ブラウザ自動操作は実行環境側の接続情報不足で開始できなかったため、実クリック確認は配信後のユーザー試験とする。
- 秘密情報を含む環境ファイルは読み取り・変更せず、`winmacsync` も実行していない。

本番反映結果:

- 実装コミット `ac0ee15a` を `main` へpushし、GitHub連携のVercel Production配信に反映した。
