...
<ONEDRIVE_FOLDER>/周辺機器/<YYYYMMDD_HHMM_親記事タイトル冒頭10文字>/へ子記事Markdownを保存
...
<ONEDRIVE_FOLDER>/
└── 周辺機器/
    └── <YYYYMMDD_HHMM_親記事タイトル冒頭10文字>/
        ├── <parent_id>_child_keyboard.md
        ├── <parent_id>_child_case.md
        └── <parent_id>_child_charger.md
```

フォルダ名の日付と時刻はジョブの `created_at` を日本時間へ変換した値とし、タイトル部分はOneDriveで使用できない文字を除去した親記事タイトルの先頭10文字とする。同一バッチは同じ `created_at` を共有するため、複数カテゴリの子記事が同じ可読フォルダへ入る。
...
6. 親記事を複製し、H1と全H2の文字列だけを決定的に変更する。
...
10. 合格した本文専用Markdownだけを `<ONEDRIVE_FOLDER>/周辺機器/<YYYYMMDD_HHMM_親記事タイトル冒頭10文字>/` へ保存し、管理JSONを制御データ領域へ保存する。日時は同一バッチのジョブ作成日時を日本時間へ変換して使う。
...
- 保存先は `<ONEDRIVE_FOLDER>/周辺機器/<YYYYMMDD_HHMM_親記事タイトル冒頭10文字>/` に固定する。同一バッチでは共通の作成日時を使い、複数カテゴリを同じ可読フォルダへまとめる。
...
- 生成件数と保存先 `<ONEDRIVE_FOLDER>/周辺機器/<YYYYMMDD_HHMM_親記事タイトル冒頭10文字>/` の確認
...
- [x] `<ONEDRIVE_FOLDER>/周辺機器/<YYYYMMDD_HHMM_親記事タイトル冒頭10文字>/` の作成と子記事だけの保存を実装する。
- [x] 制御データを記事ルート外へ保存する。
...
- `<ONEDRIVE_FOLDER>/周辺機器/<YYYYMMDD_HHMM_親記事タイトル冒頭10文字>/` のフォルダ一覧と子記事表示
