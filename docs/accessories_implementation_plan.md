...
5. 親記事を複製し、クラウドに保存済みのエンジン別プロンプトを使って、選択したエンジンだけで結論用のスペック要約とおすすめ理由を生成する。
...
9. 対象商品群の全商品、おすすめ理由、商品リンクまとめを結論だけへ追加する。
...
<ONEDRIVE_FOLDER>/周辺機器/<parent_id>/へ子記事Markdownを保存
...
周辺機器専用処理は、該当する名前付きセクションの全 `▼` ブロックを元の順番で結論内へ一回だけ挿入する。各ブロックの製品名、説明、価格表記、URL、改行は原文のまま保持し、最初の新規おすすめ商品群の直前へ既存と同じ免責事項を一回だけ付ける。

MLXとGeminiには商品群本文を自由編集させない。選択したエンジンは、親記事で確認できる仕様の短い要約と、各 `▼` ブロックに対応するおすすめ理由だけを生成する。周辺機器専用の決定的な組立処理が、理由、原文の商品ブロック、最後の商品リンクまとめを結論へ差し込む。新規追加URL集合が結論外に存在する場合は保存前検査で不合格にする。
...
6. 親製品名から既存SEO見出しの共通接頭辞を特定し、その接頭辞を持つH2だけを変換対象として印を付ける。動画見出し、キャプション、独自見出し、H3以下は変換対象にしない。
7. 変更許可範囲をH1文字列、変換対象H2文字列、結論への追記位置に限定し、それ以外の不変領域ごとにSHA-256を計算する。
...
一回の右クリック操作を一バッチとし、初期上限は5カテゴリとする。選択カテゴリごとに一つのジョブJSONと `周辺機器DB_LLM` の一行を作り、共通の `batch_id` でまとめる。大量生成時のMLX処理時間、Gemini利用量、Google Sheets API負荷、OneDrive API負荷を制御するためである。

### 6.3 記事生成

第一ゴールは「親記事派生型」とする。MLXまたはGeminiに記事全文を再生成させず、親記事を決定的に複製して、許可した三領域だけを変更する。

変更を許可する領域:

1. H1の見出し文字列
2. 結論位置以降にある、親製品名の既存SEO接頭辞を持つH2の見出し文字列
3. 結論部の末尾へ追加する仕様要約、おすすめ理由、全商品ブロック、商品リンクまとめ

H1は、既存タイトルの主製品名と接尾辞を維持しながら、次の形式へ変換する。

```text
# {製品名} {カテゴリ名}おすすめ：{既存タイトルの接尾辞}
```

変換対象H2も、見出し階層と接尾辞を維持しながら次の形式へ変換する。

```text
## {製品名} {カテゴリ名}おすすめ：{既存H2の接尾辞}
```

たとえば `battery` なら「製品名 バッテリーおすすめ：」、`cable` なら「製品名 ケーブルおすすめ：」を使う。H2数、出現順、階層は変えず、H3以下と見出し直下の本文は変更しない。カテゴリ特化の選び方、比較、FAQ、新しい章を追加しない。

選択したエンジンへ渡す本文は、親記事の結論、根拠付きの親製品仕様、番号付き商品ブロックに限定する。エンジンは、親製品の仕様要約と、各商品ブロックに一対一で対応するおすすめ理由だけを返す。MLXとGeminiはそれぞれ単独でこの応答生成と検査を完結し、片方の出力をもう片方へ渡さない。

結論の追加ブロックは次の順序に固定する。

1. 親製品の主要スペック要約
2. 対象カテゴリのおすすめ商品一覧
3. 各商品のおすすめ理由と、原文を保持した対応 `▼` ブロック
4. 商品名と原文URLだけを並べたリンクまとめ

対象セクションの全 `▼` ブロックを一件も省略せず、原文順で一回だけ掲載する。新たに追加するカテゴリURLは結論内だけに存在させる。親記事に元からあるURLは、結論外を含めて元の位置と内容を維持する。

MLX実装では既存 `Step00` のSEO管理値生成を、記事本文へ出さない管理JSON用入力としてだけ安全に再利用できる。既存の全文記事生成は再利用せず、結論用要約・理由の専用呼び出しと検査を実装する。Geminiも同じ限定入出力契約とし、不合格時は選択中の同一エンジンだけで再試行する。
...
4. 選択したMLXまたはGeminiだけで、仕様要約と商品ブロック番号ごとのおすすめ理由を生成する。
5. 理由の件数、参照番号、入力根拠、禁止内容を検査する。
6. 親記事を複製し、H1と変換対象H2の文字列だけを決定的に変更する。
7. 仕様要約、おすすめ理由、原文商品ブロック、リンクまとめを結論末尾へ決定的に挿入する。通常記事用 `insert_affiliate_links.py` は呼ばない。
8. 不変領域のSHA-256、見出し数・階層・順序、商品ブロック全文・順序、新規URLの位置、内部思考、Frontmatter、YAML、JSON、管理値の非混入を最終検査する。
9. `locked_seo`、ジョブ、プロンプト、親子関係を記事ルート外の子記事管理JSONへ組み立てる。
10. 合格した本文専用Markdownだけを `<ONEDRIVE_FOLDER>/周辺機器/<parent_id>/` へ保存し、管理JSONを制御データ領域へ保存する。
...
- 保存先は `<ONEDRIVE_FOLDER>/周辺機器/<parent_id>/` に固定する。
...
- 右クリックした一記事を親記事として固定した専用モーダルを開く。
- 既存の記事複数選択状態があっても、周辺機器生成では `getContextTargetIds` の複数対象を使わず、`ctxMenu.article` の一記事だけを親記事にする。
- 検出カテゴリを複数選択できるチェックボックスを表示する。
- 各カテゴリが参照する `===battery===`、`===cable===` などの商品群名と商品件数
- 実行前の商品群プレビュー
- 使用するエンジン別プロンプト名と改訂番号
- 生成件数と保存先 `<ONEDRIVE_FOLDER>/周辺機器/<parent_id>/` の確認
...
親子関係と生成エンジン表示は、`周辺機器/<parent_id>/` の保存フォルダ、記事ルート外の親・子管理JSON、`周辺機器DB_LLM` のジョブIDを利用する。記事Markdownへ管理情報を埋め込まず、一覧取得時に全記事本文を読む負荷も避ける。
...
- [x] H1・既存SEO形式H2の接尾辞を保持するカテゴリ見出し変換ルールを固定する。
...
- [x] MLX用・Gemini用プロンプトの出力を、仕様要約と商品ブロック別おすすめ理由だけに限定する。
...
- サンプルのH1と既存SEO形式H2が、接尾辞を保持した「製品名 カテゴリ名おすすめ：」形式へ変換される。
- H1、対象H2、結論追記以外の親記事本文がバイト単位で一致する。
...
- [x] `<ONEDRIVE_FOLDER>/周辺機器/<parent_id>/` の作成と子記事だけの保存を実装する。
- [x] 制御データを記事ルート外へ保存する。
...
- [x] 親記事の結論・根拠付き仕様・番号付き商品ブロックを受け取り、仕様要約と商品別おすすめ理由だけを返すMLX専用エンジンを実装する。
...
- エンジン結果が仕様要約と商品ブロック別おすすめ理由だけであり、記事全文、見出し、URL、管理メタ情報を含まないこと
...
- `<ONEDRIVE_FOLDER>/周辺機器/<parent_id>/` のフォルダ一覧と子記事表示
- 記事ルート外の制御データ保存と、記事Markdownへの管理情報非混入
- 親記事と子記事の構造差分がH1、対象H2、結論追記だけであること
...
| SEO形式でない見出しまで書き換える | 動画、キャプション、独自章の意味が壊れる | 親製品名から特定した既存SEO接頭辞を持つH2だけを変換し、H3以下とその他見出しを不変検査する |
...
| フェーズ3 | 進行中 | 2026-08-12 | Vercel認証、機密環境変数登録、本番相当ビルド、OneDrive並行初期化の安定化を実施。デプロイとユーザー試験を残す |
