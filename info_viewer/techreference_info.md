...
## 1. この文書の目的
この文書は、`%USERPROFILE%\OneDrive\開発\Blog_Vercel\info_viewer` に関する実装メモです。  
...
`%USERPROFILE%\OneDrive\開発\Blog_Vercel\api\info-viewer.js` は、候補フォルダの中から最初に見つかった `manifest.json` / `pipeline_state.json` を使う実装だった。  
...
## 5. 修正内容
### 5.1 workflow の保存先統一
`%USERPROFILE%\OneDrive\開発\Blog_Vercel\.github\workflows\info-viewer-queue.yml` の
...
`%USERPROFILE%\OneDrive\開発\Blog_Vercel\api\info-viewer.js` を修正し、候補の中から
...
- `%USERPROFILE%\OneDrive\開発\Blog_Vercel\scripts\info_viewer\modules\gemini_formatter.py`
- `%USERPROFILE%\OneDrive\開発\Blog_Vercel\scripts\info_viewer\runner.py`
...
`%USERPROFILE%\OneDrive\開発\Blog_Vercel\.github\workflows\info-viewer-pipeline.yml` は legacy 扱い。  
