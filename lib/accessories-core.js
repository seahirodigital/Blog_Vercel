...
export function parseAffiliateText(text, sectionName) {
  ...
  const exactIndex = matches.findIndex((match) => match[1] === sectionName);
  const index = exactIndex >= 0
    ? exactIndex
    : matches.findIndex((match) => affiliateSectionNamesEqual(match[1], sectionName));
...
const AFFILIATE_SECTION_EXTRA_ALIASES = {
  adapter: ['アダプター'],
};

function normalizedAffiliateSectionName(value) {
  return String(value || '').normalize('NFKC').trim().toLocaleLowerCase('ja-JP');
}

function affiliateSectionCanonicalName(value) {
  const normalized = normalizedAffiliateSectionName(value);
  for (const [id, label] of Object.entries(AFFILIATE_SECTION_LABELS)) {
    const aliases = [id, label, ...(AFFILIATE_SECTION_EXTRA_ALIASES[id] || [])];
    if (aliases.some((alias) => normalizedAffiliateSectionName(alias) === normalized)) return label;
  }
  return String(value || '').normalize('NFKC').trim();
}

export function affiliateSectionNamesEqual(left, right) {
  return normalizedAffiliateSectionName(affiliateSectionCanonicalName(left))
    === normalizedAffiliateSectionName(affiliateSectionCanonicalName(right));
}
...
export function affiliateSectionLabel(sectionId) {
  ...
  return affiliateSectionCanonicalName(id);
}
...
export function normalizeAffiliateSectionsForEditing(sections) {
  const seen = new Set();
  return sections.map((section) => {
    const sourceId = String(section?.id || '').trim();
    const memo = /^MEMO\d+$/i.test(sourceId);
    const id = memo ? sourceId : affiliateSectionLabel(sourceId);
    const duplicateKey = normalizedAffiliateSectionName(id);
    if (seen.has(duplicateKey)) {
      throw new Error(`タブ名の統一後にセクション名が重複します: ${id}`);
    }
    seen.add(duplicateKey);
    return {
      id,
      label: memo ? affiliateSectionLabel(sourceId) : id,
...
export function serializeAffiliateSections(sections) {
  ...
  const chunks = sections.map((section) => {
    ...
    if (!/^[\p{L}\p{N}_-]+$/u.test(id)) throw new Error(`セクション名が不正です: ${id}`);
    const duplicateKey = normalizedAffiliateSectionName(id);
    if (seen.has(duplicateKey)) throw new Error(`セクション名が重複しています: ${id}`);
    seen.add(duplicateKey);
