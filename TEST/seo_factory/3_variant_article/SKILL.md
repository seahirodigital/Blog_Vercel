...
1. `../output/<seed>/memo/031_3_master_validation_report.md` が合格済みであることを確認する。
2. `../0_common/scripts/031_5_run_factory.py --resume-from-sheet` を実行し、`../output/<seed>/memo/031_4_kobetsu_jobs.md` を生成する。
3. `prompts/031-4-kobetsu-writer-prompt.md` と `../0_common/promptreference.md` を確認し、対象キーワードごとの必須 H2 と禁止事項を揃える。
4. 各ジョブに従って `../output/<seed>/variants/<target_keyword>.md` をこのチャットで執筆する。
5. 必要数を書いたら `../0_common/scripts/031_3_article_validator.py` を使って個別記事検証を行う。
6. `../output/<seed>/memo/031_4_variant_validation_report.md` が合格するまで修正を繰り返す。
