 ...
 * GET    /api/xpost-blog-articles         → 記事一覧取得（サブフォルダ再帰対応）
 * GET    /api/xpost-blog-articles?id=xxx  → 記事内容取得
 * PUT    /api/xpost-blog-articles         → 記事保存（新規 or 上書き）
 * PATCH  /api/xpost-blog-articles         → 記事リネーム
 *
 * ★ トークン自動ローテーション実装済み
 */
...
async function saveArticle(token, filename, content, fileId = null) {
  ...
  if (fileId) {
    ...
    // 新規ファイル: ルートフォルダに作成
    const folder = process.env.XPOST_BLOG_ONEDRIVE_FOLDER || 'Obsidian in Onedrive 202602/Vercel_Blog/X投稿';
...
async function moveArticle(token, fileId, destRelativePath) {
  const baseFolder = process.env.XPOST_BLOG_ONEDRIVE_FOLDER || 'Obsidian in Onedrive 202602/Vercel_Blog/X投稿';
